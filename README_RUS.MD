...
Существует два метода расширения ключа, однако их названия не стандартизированы и они могут именоваться по разному:

| Атрибут                                                 | Метод 1: Cisco/Reeder                                    | Метод 2: Blumenthal-Wiener              |
|---------------------------------------------------------|----------------------------------------------------------|-----------------------------------------|
| **Параметр в этой библиотеке**                          | `aes192`, `aes256`                                       | `aes192a`, `aes256a`  (*a* = AGENT++)   |
| **Имя в Wireshark**                                     | `draft-reeder-snmpv3-usm-3desede-00`                     | `AGENT++`                               |
| **Распространенное название**                           | Cisco, Reeder, Draft-Reeder                              | Blumenthal, Blumenthal-Wiener           |
| **Параметр в прочих библиотеках**                       | `aes192c`, `aes256c` (*c* = Cisco)                       | `aes192`, `aes256`                      |
| **Имя IETF но draft**                                   | `draft-reeder-snmpv3-usm-3desede-00`, 3DES-EDE Extension | `draft-blumenthal-aes-usm-04`           |
| **Типичные имена у производителей оборудования или ПО** | Cisco Style, Cisco Extension, Standard                   | AGENT++, AgentPlusPlus, Alternative AES |
| **Заметки**                                             | Самый распространенный метод, поэтому без суффикса       | Используется в коммутаторах Huawei      |
...
```
### Настройка коммутатора для выполнения тестов, на примере Cisco Catalyst
...
snmp-server user snmpuser SNMPv3-G v3 auth sha auth12345 priv aes 128 priv12345
```
