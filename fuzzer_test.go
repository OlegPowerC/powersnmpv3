...
func fuzzSessionV2() *SNMPv3Session {
	s := &SNMPv3Session{}
...
func FuzzReceiverV2(f *testing.F) {
	...
	nullVB := []SNMP_Packet_V2_VarBind{{oidSys, ASNber.NullRawValue}}
	multiVB := []SNMP_Packet_V2_VarBind{
		{oidSys, ASNber.NullRawValue},
		{oidIf, Convert_setvar_toasn1raw(SetSNMPVar_OctetString("fuzz-seed"))},
	}
	// Exception-varbind noSuchObject: context-specific, tag 0, пустое значение
	excVB := []SNMP_Packet_V2_VarBind{{oidSys,
		ASNber.RawValue{Class: ASNber.ClassContextSpecific, Tag: tagERR_noSuchObject}}}

	seeds := []struct {
		vbs        []SNMP_Packet_V2_VarBind
		reqType    int
		nr, maxRep int32
	}{
		{nullVB, SNMPv2_REQUEST_GET, 0, 0},
		{nullVB, SNMPv2_REQUEST_GETNEXT, 0, 0},
		{multiVB, SNMPv2_REQUEST_RESPONSE, 0, 0},
		{multiVB, SNMPv2_REQUEST_SET, 0, 0},
		{nullVB, SNMPv2_REQUEST_GETBULK, 0, 25},
		{multiVB, 6, 0, 0}, // InformRequest
		{multiVB, 7, 0, 0}, // SNMPv2-Trap
		{excVB, SNMPv2_REQUEST_RESPONSE, 0, 0},
	}
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		_, _, _, _ = ParseTrapUsername(data)
	})
}
