...
func TestConvert_Variable_To_String(t *testing.T) {
	tests := []struct {
		name string
		var_ PowerSNMPv3.SNMPVar // ← var_ вместо var
		want string
	}{
		{
			name: "OctetString",
			var_: PowerSNMPv3.SNMPVar{ValueClass: 0, ValueType: 4, Value: []byte("TestVal123")},
			want: "TestVal123",
		},
		{
			name: "INTEGER",
			var_: PowerSNMPv3.SNMPVar{ValueClass: 0, ValueType: 2, Value: []byte{0, 7}},
			want: "7",
		},
		{
			name: "IPADDR",
			var_: PowerSNMPv3.SNMPVar{ValueClass: 1, ValueType: 0, Value: []byte{192, 168, 21, 119}},
			want: "192.168.21.119",
		},
		{
			name: "TIMETICKS",
			var_: PowerSNMPv3.SNMPVar{ValueClass: 1, ValueType: 3, Value: []byte{120}},
			want: "1.2s",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := PowerSNMPv3.Convert_Variable_To_String(tt.var_)
			if got != tt.want {
...
func TestConvert_ClassTag_to_String(t *testing.T) {
	TypeIs := PowerSNMPv3.Convert_ClassTag_to_String(PowerSNMPv3.SNMPVar{ValueClass: 1, ValueType: 1})
	if TypeIs != "COUNTER32" {
...
func TestConvert_bytearray_to_int(t *testing.T) {
	intcoverted := PowerSNMPv3.Convert_bytearray_to_int([]byte{255, 255, 248, 148})
	if intcoverted != -1900 {
	...
	intcoverted2 := PowerSNMPv3.Convert_bytearray_to_int([]byte{195})
	if intcoverted2 != -61 {
...
func TestConvert_bytearray_to_uint(t *testing.T) {
	intcoverted := PowerSNMPv3.Convert_bytearray_to_uint([]byte{255, 248, 148})
	...
	intcoverted2 := PowerSNMPv3.Convert_bytearray_to_uint([]byte{195})
	if intcoverted2 != 195 {
...
func TestConvert_OID_IntArrayToString_DER(t *testing.T) {
	...
	Str2 := PowerSNMPv3.Convert_OID_IntArrayToString_DER(PowerSNMPv3.Convert_bytearray_to_intarray(TestIntArry))
...
func TestConvert_OID_IntArrayToString_RAW(t *testing.T) {
	...
	Str2 := PowerSNMPv3.Convert_OID_IntArrayToString_RAW(PowerSNMPv3.Convert_bytearray_to_intarray(TestIntArry))
...
func TestConvert_bytearray_to_intarray_with_multibyte_data(t *testing.T) {
	...
	IntArry := PowerSNMPv3.Convert_bytearray_to_intarray_with_multibyte_data(TestIntArry)
...
func TestConvert_snmpint_to_int32(t *testing.T) {
	...
	ConvertetData := PowerSNMPv3.Convert_snmpint_to_int32(TestInt16SignedArray)
	if ConvertetData != -2511 {
		t.Errorf("Get value %d", ConvertetData)
	}
	TestInt16SignedArray2 := []byte{195}
	ConvertetData2 := PowerSNMPv3.Convert_snmpint_to_int32(TestInt16SignedArray2)
...
func TestConvert_snmpint_to_uint32(t *testing.T) {
	TestInt16SignedArray := []byte{0xf6, 0x31}
	ConvertetData := PowerSNMPv3.Convert_snmpint_to_uint32(TestInt16SignedArray)
	if ConvertetData != 63025 {
		t.Errorf("Get value %d", ConvertetData)
	}
	TestInt16SignedArray2 := []byte{195}
	ConvertetData2 := PowerSNMPv3.Convert_snmpint_to_uint32(TestInt16SignedArray2)
...
func TestConvert_Covert_OID_StringToIntArray_DER(t *testing.T) {
	TestStrOid := ".1.3.6"
	ConvertetData, _ := PowerSNMPv3.Convert_OID_StringToIntArray_DER(TestStrOid)
...
func TestParseOID(t *testing.T) {
	...
	_, ConvErr := PowerSNMPv3.ParseOID(TestStrOid)
	if ConvErr == nil {
	...
	intoid, cverr := PowerSNMPv3.ParseOID(TestStrOid)
	if cverr != nil {
		t.Error(ConvErr)
	}
