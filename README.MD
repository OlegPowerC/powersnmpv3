...
- Encryption: DES, 3DES, AES-128, AES-192, AES-256 (Blumenthal, Cisco/Reeder AES-C, and AGENT++ variants)
...
Validates SNMP device configuration parameters.  
Called by SNMP_Init, but can also be called independently.
...
**Returns:**
- `error` -  error or nil (can be structured error or simple error)
...
Validates SNMPv3 string parameters and returns numeric constants.  
Useful for building a map of user parameters in SNMP trap receivers
...
```
**Parameters:**
- `authproto`: authentication protocol. Supported: "","**none**", "**md5**", "**sha**", "**sha224**", "**sha256**", "**sha384**", "**sha512**".
- `authkey`: authentication passphrase. Required if authproto is set and != "**none**". Minimum length: **SNMP_AUTH_PRIV_KEY_MINLEN** characters.
- `privproto`: privacy protocol. Supported: "**none**", "**des**", "**3des**", "**aes**", "**aes192**", "**aes256**", "**aes192a**", "**aes256a**", "**aes192c**", "**aes256c**".
- `privkey`: privacy passphrase. Required if privproto is set and != "**none**". Minimum length: **SNMP_AUTH_PRIV_KEY_MINLEN** characters.
- Empty strings for authproto/privproto are treated as "**none**".

**Returns:**
- `seclevelInt`: SNMPv3 security level: 0 = NoAuthNoPriv, 1 = AuthNoPriv, 2 = AuthPriv
- `authprotoInt`: auth protocol constant
- `privprotoInt`: priv protocol constant
- `err`: validation error of type **SNMPSetparameters_Errors**, or **nil** on success. Returned int values are only valid if err == nil.
...
**Example:**
```
oids := []PowerSNMP.SNMP_Packet_V2_Decoded_VarBind{
...
oid, _ := PowerSNMP.ParseOID("1.3.6.1.2.1.2.2.1.2")
...
- **PowerSNMPv3** returns a partial error which contains information about which OIDs are noSuchObject
and returns result without failed OIDs

In **Set** operation:  

- **gosnmp** returns result with VarBinds which it tried to set and no error
Result may be incorrectly interpreted by user.
User must explicitly check resultSet.Error,resultSet.ErrorIndex to detect error

- **PowerSNMPv3** returns a fatal error which contains the first failed OID and type of error
and no result. It is really a fatal error, because the SET operation is atomic
...
use the **checkparams** tag for test parameters validation functions.  
use the **integration** tag for test with real device.
...
go test -v -tags=integration -args \
    ...
    -c <COMMUNITY>
```
To ensure all integration tests pass, the community string must have RW permissions,
and the SNMPv3 user must also have read and write access.
...
1. **Unit tests** (no device required):
   ...
   - `TestCheckUserParams` - validation snmp parametesrs
   - `TestSetAuthPrivParamsStToInt` - validation/conversion snmp auth/priv parametesrs
   - `TestCheckParamsIPandPortSNMP` - validation ip, port
   - `TestBasicCheckParamsVersionSNMP` - validation SNMP version check
   - `TestBasicCheckParamsVersion2SNMP` - validation SNMP v2 parameters
...
```
### Switch configuration for running the test (Cisco Catalyst example)
